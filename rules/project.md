...
## Finance見込みの「数値事実のみ」不変条件は行単位でスコープする（v0.8 2-1・サイクル8.2）

Cycle 2.2で導入した「不足行は結果断定・警告記号を含まない数値事実のみ」というFinance見込みの
不変条件（担保: `tests/test_prompt_finance.py`）は、当初`assert "⚠" not in text`（プロンプト
全体）として実装されていた。v0.8仕様2-1はFinance見込みブロックに「⚠ 今Rで市場賞金
{shortfall}円以上を得られなければ、強制返済不能で破産脱落します」という独立した警告行の追加を
要求しており、プロンプト全体を対象にした素朴な実装のままでは正面から衝突する。

サイクル8.2でこれを「Finance見込みの行（`今R賞金0の場合の現金見込み: ...`）そのものには⚠と
評価語が無いこと」へスコープし直した（テストのassertionをプロンプト全体走査から対象行の
`in`チェックへ変更）。これにより「数値見込みの行自体は事実のみで完結する」というCycle 2.2の
意図を保ったまま、仕様が要求する独立警告行を別行として許可できる。今後、数値行に隣接する
警告・注意書きを追加したくなった場合は、対象行を混在させず必ず別行に分離すること。新しい
