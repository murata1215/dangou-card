...
（仕様書の役割分担: `..._v0_8_season2.md`＝2026-08-16時点の実装から起こした完全版、
`..._v0_8_regulation_season2.md`＝2026-08-30のレギュレーション改訂差分、
`..._v0_9_regulation_season2.md`＝2026-08-30のFree Cash廃止差分、
`..._v0_10_typec.md`＝2026-09-19の型C（条件付き金銭契約）追加差分（最新の正本）。
四者が食い違う場合は v0.10 > v0.9 > v0.8 regulation > 完全版 の順で優先する）
...
「未到来（期限が現在R以降）かつ未履行かつ未失効」であるものが1件以上あることから毎回導出
される（v0.8で条件を緩和し、履行/監査済みの過去義務があっても未到来義務が残っていれば
解除できるようにした）。片側だけが同意した状態では契約台帳に`⏳ 解除同意 1/2`のように
表示され、全員揃うまで`ACTIVE`のまま。解除された契約の未到来義務は失効し、Settlement・
型B監査・型A執行・脱落判定の既存ロジック（`status != ACTIVE`で弾く）にそのまま乗るため
対象外になる。全ての義務が消化済みになったACTIVE契約はSettlement/Finance後に自動的に
`CLOSED`へ遷移し、署名待ち（`PROPOSED`）のまま提案ラウンドが終わった契約は`EXPIRED`へ
遷移する。詳細は`rules/project.md`「§`ACTIVE`からの実遷移」を参照。
...
### v0.8プロンプト文面の一括修正（サイクル8.2）

サイクル8.1のエンジン差分（契約無料化・署名待ち失効・トレード枠の成立ベース化・公開情報4キー・
倍掛けpayoutのFinance連動）に合わせ、`llm/prompt_builder.py`の出力文面を更新した。Finance見込みは
「今R賞金0の場合の現金見込み: 現金 − Entry Fee − 強制返済 = 結果」の1行式に統一し、不足時は
独立した警告行を追加する。交渉プロンプトには倍掛け中のプレイヤー・各プレイヤーの使用済みカード・
正式契約の状況（公示）・成立済みカードトレードの4公開情報ブロックを追加し、署名待ち契約には
「署名した場合、あなたが受け取るもの」プレビューを追加した。倍掛け選択プロンプトには
「DOUBLEを選んだ場合の次ラウンド見通し」ブロックを新設し、次Rの強制最低返済に対する不足を
事前に警告する。詳細は`doc/devlog/2026-08-30_054059.md`と`doc/prompt_dump/actual_prompts_20260830_v2.md`
（最新の実プロンプト全文ダンプ）を参照。
