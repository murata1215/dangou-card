...
>
> **2026-08-17 是正済み**: `engine/game.py` に `_visible_messages()` を新設し、DM は送信者・宛先以外に対し `message` キーごと削除して返すよう修正した（reasoning/memory と同じ構造的秘匿）。非当事者には `[P01→P02] （非公開のDM）` とメタデータのみ表示される。詳細は `doc/changelog.md` 2026-08-17「DM本文の秘匿化」参照。
