...
倍掛けpayoutのFinance連動）をLLMが読む文面へ反映した。`llm/prompt_builder.py`のみ変更（`engine/`
は無変更）。2セッションに分かれて実施（前セッションでsystem prompt/RULES_SUMMARYを完了）。

- **system prompt**: 脱落理由を3種別→4種別（破産をEntry Fee/強制返済の2種に分割）に変更、
  正式契約説明に「無料。自動執行/自動監査」を明記、型A支払原資（市場賞金・前R倍掛け払出が
  同R型A支払原資になる旨）を追記。
- **negotiation prompt**: Finance見込みを「今R賞金0の場合の現金見込み: {cash}円 − Entry Fee
  {fee}円 − 強制返済 {repay}円 = {result}円」の1行式へ統合（fee=0時は区間省略、負なら別行で
  ⚠警告）。倍掛け中のプレイヤー／各プレイヤーの使用済みカード／正式契約の状況（公示）／
  成立済みカードトレードの4公開情報ブロックを追加。署名待ち契約に「署名した場合、あなたが
  受け取るもの」プレビューと自分の義務合流プレビューを追加。トレード提案表示を
  「Xが渡す: A ／ Yが渡す: B ＋ 現金C円（誰が払うか）」形式へ統一。
- **double_up prompt**: 「2倍払出は次Rの型A支払原資になる」「成功賞金は再DOUBLE不可」の2文を
  追加、「DOUBLEを選んだ場合の次ラウンド見通し」ブロックを新設（DOUBLE失敗・次R市場賞金0を
  仮定した最低ラインで次Rの強制最低返済額とEntry Fee控除後現金を概算、不足時は⚠警告）。
- **commit/reflection prompt**: Finance行を上記1行式へ統一、公開4ブロックを展開、契約ステータス
  語を「有効/完了/解除/失効/終了」へ統一、reflectionの次R予測Financeに⚠赤字警告を追加。
- 既存失敗テスト22件を期待文字列・定数更新のみで是正（アサーション強度は不変）。特にFinance
  ⚠不使用ルール（Cycle 2.2由来）は「見込み行そのもの」へスコープを絞り、独立警告行を許可する
  形へ改めて、仕様2-1の要求と両立させた。新規`tests/test_prompt_v08.py`（19件）追加。
- `doc/prompt_dump/actual_prompts_20260830_v2.md`を新規生成（v1と同一のダミーシナリオ・手法。
  「発火している条件付きブロック」欄は出力テキストの見出しをgrepで機械抽出する方式に変更、
  v1の手書き誤り2件を訂正注記としてv1冒頭に追記）。
- `uv run pytest -q`は**1533 passed**（前1514+19）。`scripts/dry_run.py`は578イベントで完走
  （不変）。`logs/llm/`は47件のまま不変。`git diff --stat engine/`は空。system prompt文字数は
  `baseline_v1_s2`で8,177字・`baseline_v1`で8,105字（いずれも≤8,300）、H1予約額0.03459（≤0.035）。
  詳細: `doc/devlog/2026-08-30_054059.md`。
