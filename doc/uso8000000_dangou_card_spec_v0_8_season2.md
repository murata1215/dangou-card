...
> ※ 2026-08-30 のレギュレーション改訂（契約無料化・提案失効・解除条件・倍掛け順序・トレード制限ほか）は `doc/uso8000000_dangou_card_spec_v0_8_regulation_season2.md` を参照。食い違う箇所は同文書が優先。
...
- 生存者一覧
- 直近のメッセージ履歴（broadcast + **自分が当事者のDM本文**、直近10件。非当事者のDMは
  `[送信者→宛先] （非公開のDM）`とメタデータ（送信者・宛先・turn）のみ表示され本文は伏せられる。
  匿名通信（`anonymous_broadcast`）は本文が全員に公開されるが掲載者は秘匿。
  2026-08-17: これまでDM本文が`for_player_id`を無視して全員に描画されていた（§8.2「DM内容」秘匿の
  未実装）実装漏れの是正。あわせて、匿名通信が`_round_messages`に一切積まれておらず本文が誰にも
  届いていなかった不具合（§7.1機能不全）も修正）
- **自分が当事者**の提案中契約（`contracts_pending`）・カードトレード提案
- 使用済みカード一覧（全プレイヤー分・公開情報）
- **自分自身**の署名済み未履行契約義務一覧（`_render_obligations_block()` によりnegotiation/commit/double_upの3フェーズ全てに注入。現ラウンドの義務は強調表示され、同一ラウンドに矛盾するtype_b_card義務が複数ある場合は警告が出る。2026-08-17: commit/double_upは`_build_visible_state()`呼び出しに`for_player_id`が渡っていなかったため`my_obligations`キー自体が生成されず、実際にはnegotiationフェーズにしか注入されていなかった実装漏れを是正）
...
**含まれないもの**: 他プレイヤーの現金・借金・Free Cash・未使用手札・**DM内容（非当事者。`_visible_messages()`が`message`キーごと削除して返すため構造的に混入しない）**・非当事者の契約義務詳細・**他プレイヤーの reasoning**・**他プレイヤーの引き継ぎメモリ**（`memory`はエージェントインスタンス内に閉じ、`visible_state`を経由しないため構造的に混入しない）。
