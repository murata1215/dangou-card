...
| `/watch` | 観戦ビューア本体。相対パス(`api/...`/`static/...`)依存のため**末尾スラッシュ無し**で配信 |
| `/static/*` | 静的ファイル（LPヒーロー `lp_hero.png` 含む） |
| `/api/*` | 試合データAPI |

LP: `viewer/static/lp.html`（自己完結・インラインCSS）。観戦UI: `viewer/static/index.html`。

## 画面

- **LP(`/`)**: ヒーロー(タイトル＋2大CTA) / 概要3カード / ブログ記事ピックアップ / フッタ
- **ヘッダ(`/watch`)**: 🏠トップ / 試合セレクタ / ラウンド / 生存者数 / 総コスト / 自動更新トグル
